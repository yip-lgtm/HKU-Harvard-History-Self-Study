...
```mermaid
graph TD
    A[1750 前：前現代全球化<br/>絲綢之路、香料之路] --> B[1780-1870：第一波工業全球化<br/>英國主導、鐵路與蒸汽船]
    B --> C[1870-1914：帝國主義全球化<br/>歐洲瓜分非洲與亞洲]
    C --> D[1914-1945：去全球化<br/>兩次世界大戰、貿易壁壘]
    D --> E[1945-1989：冷戰全球化<br/>美國 vs 蘇聯體系]
    E --> F[1989-2020：新自由主義全球化<br/>跨國公司、互聯網]
    F --> G[2020-：碎片化<br/>脫鈎、供應鏈重組]
```
...
```mermaid
graph TD
    A[單一現代性線性模型] -->|已過時| B[多元現代性並存]
    B --> C[西歐模式：自由民主 + 市場經濟]
    B --> D[東亞模式：威權發展型國家 + 出口導向]
    B --> E[蘇聯模式：計劃經濟 + 一黨專政]
    B --> F[伊斯蘭模式：宗教法律 + 有限市場]
    style B fill:#FFD700
```
...
```mermaid
graph LR
    A["方法論選擇"] --> B["最相似系統<br/>（香港 vs 新加坡）"]
    A --> C["最大差異系統<br/>（英國 vs 日本）"]
    A --> D["歷史過程追蹤<br/>（Path Dependence）"]
    B --> E["控制變量：殖民統治者、地理"]
    C --> F["暴露差異：文化路徑、危機處理"]
    D --> G["關鍵轉折點識別"]
```
...
graph TD
    subgraph 世界體系
        A[核心國家<br/>Core] --> B[半邊緣<br/>Semi-Periphery]
        B --> C[邊緣國家<br/>Periphery]
        A --> C
    end
    A -->|"技術、資本、高利潤"| A
    B -->|"原材料、廉價勞動力"| A
...
```mermaid
graph TD
    A["1850 單一現代性話語<br/>（西方 = 普世）"] --> B["1900 帝國主義話語"]
    A --> C["1950 冷戰話語<br/>（資本主義 vs 共產主義）"]
    C --> D["1989 福山：歷史終結"]
    C --> E["1996 亨廷頓：文明衝突"]
    D --> F["2008 金融危機<br/>質疑新自由主義"]
    E --> F
    F --> G["2020s 多元現代性話語復興<br/>（中國、印度、伊斯蘭模式）"]
    style G fill:#FFD700
```
...
graph TD
    A["歷史問題"] --> B["選擇方法"]
    B --> C["最相似系統設計<br/>香港 vs 新加坡"]
    B --> D["最大差異系統設計<br/>英國 vs 日本"]
    B --> E["過程追蹤<br/>Path Dependence"]
    C --> F["控制變量：殖民、港口、英語"]
    D --> G["識別關鍵差異：文化、危機、地理"]
    E --> H["識別轉折點：1930s、1960s、1997"]
    F --> I["比較結論"]
    G --> I
    H --> I
    style I fill:#90EE90
